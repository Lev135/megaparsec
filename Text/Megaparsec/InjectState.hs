...
module Text.Megaparsec.InjectState where
...
import Text.Megaparsec.Internal hiding
  (ParsecT, unParser, accHints, withHints, runParsecT, toHints, refreshHints)
import Text.Megaparsec.Pos

-- | @'ParsecT' e s m a@ is a parser with custom data component of error
-- @e@, stream type @s@, underlying monad @m@ and return type @a@.
newtype ParsecT st e s m a = ParsecT
  { unParser ::
      forall b.
      State s e ->
      (a -> State s e -> Hints (Token s) -> st -> m (b, st)) -> -- consumed-OK
      (ParseError s e -> State s e -> st -> m (b, st)) -> -- consumed-error
      (a -> State s e -> Hints (Token s) -> st -> m (b, st)) -> -- empty-OK
      (ParseError s e -> State s e -> st -> m (b, st)) -> -- empty-error
      st -> m (b, st)
  }

-- | @since 5.3.0
instance (Stream s, Semigroup a) => Semigroup (ParsecT st e s m a) where
  (<>) = liftA2 (<>)
  {-# INLINE (<>) #-}
...
-- | @since 5.3.0
instance (Stream s, Monoid a) => Monoid (ParsecT st e s m a) where
  mempty = pure mempty
  {-# INLINE mempty #-}
  mappend = (<>)
  {-# INLINE mappend #-}
...
instance
  ...
  IsString (ParsecT st e s m a)
  where
  fromString s = tokens (==) (fromString s)

instance Functor (ParsecT st e s m) where
  fmap = pMap

pMap :: (a -> b) -> ParsecT st e s m a -> ParsecT st e s m b
pMap f p = ParsecT $ \s cok cerr eok eerr ->
  unParser p s (cok . f) cerr (eok . f) eerr
{-# INLINE pMap #-}

-- | 'pure' returns a parser that __succeeds__ without consuming input.
instance (Stream s) => Applicative (ParsecT st e s m) where
  pure = pPure
  (<*>) = pAp
  p1 *> p2 = p1 `pBind` const p2
  p1 <* p2 = do x1 <- p1; void p2; return x1

pPure :: (Stream s) => a -> ParsecT st e s m a
pPure x = ParsecT $ \s _ _ eok _ -> eok x s mempty
{-# INLINE pPure #-}

pAp ::
  (Stream s) =>
  ParsecT st e s m (a -> b) ->
  ParsecT st e s m a ->
  ParsecT st e s m b
...
instance (Ord e, Stream s) => Alternative (ParsecT st e s m) where
  empty = mzero
  (<|>) = mplus

-- | 'return' returns a parser that __succeeds__ without consuming input.
instance (Stream s) => Monad (ParsecT st e s m) where
  return = pure
  (>>=) = pBind
...
instance (Stream s) => Fail.MonadFail (ParsecT st e s m) where
  fail = pFail

pFail :: String -> ParsecT st e s m a
pFail msg = ParsecT $ \s@(State _ o _ _) _ _ _ eerr ->
  let d = E.singleton (ErrorFail msg)
   in eerr (FancyError o d) s
{-# INLINE pFail #-}

instance (Stream s, MonadIO m) => MonadIO (ParsecT st e s m) where
  liftIO = lift . liftIO

-- instance (Stream s, MonadReader r m) => MonadReader r (ParsecT st e s m) where
--   ask = lift ask
--   local f p = mkParsecT $ \s -> local f (runParsecT p s)

-- instance (Stream s, MonadState st m) => MonadState st (ParsecT st e s m) where
--   get = lift get
--   put = lift . put

instance (Stream s) => MonadState st (ParsecT st e s m) where
  get = ParsecT $ \s _ _ eok _ st -> eok st s mempty st
  put st = ParsecT $ \s _ _ eok _ _ -> eok () s mempty st

-- instance (Stream s, MonadCont m) => MonadCont (ParsecT st e s m) where
...
-- instance (Stream s, MonadError e' m) => MonadError e' (ParsecT st e s m) where
...
mkParsecT ::
  ...
  ParsecT st e s m a
mkParsecT k = ParsecT $ \s cok cerr eok eerr st -> do
  (Reply s' consumption result) <- k s
  case consumption of
    Consumed ->
      case result of
        OK hs x -> cok x s' hs st
        Error e -> cerr e s' st
    NotConsumed ->
      case result of
        OK hs x -> eok x s' hs st
        Error e -> eerr e s' st
{-# INLINE mkParsecT #-}
...
--
-- > try v >> mzero = mzero
instance (Ord e, Stream s) => MonadPlus (ParsecT st e s m) where
  mzero = pZero
  mplus = pPlus

pZero :: ParsecT st e s m a
...
-- | @since 6.0.0
-- instance (Stream s, MonadFix m) => MonadFix (ParsecT st e s m) where
...
instance (Stream s) => MonadTrans (ParsecT st e s) where
  lift amb = ParsecT $ \s _ _ eok _ st ->
    amb >>= \a -> eok a s mempty st

instance (Ord e, Stream s) => MonadParsec e s (ParsecT st e s m) where
...
pLabel :: String -> ParsecT st e s m a -> ParsecT st e s m a
...
pWithRecovery ::
  (Stream s) =>
  (ParseError s e -> ParsecT st e s m a) ->
  ParsecT st e s m a ->
  ParsecT st e s m a
...
pObserving ::
  (Stream s) =>
  ParsecT st e s m a ->
  ParsecT st e s m (Either (ParseError s e) a)
...
pEof :: forall st e s m. (Stream s) => ParsecT st e s m ()
...
pGetParserState :: (Stream s) => ParsecT st e s m (State s e)
pGetParserState = ParsecT $ \s _ _ eok _ -> eok s s mempty
{-# INLINE pGetParserState #-}

pUpdateParserState :: (Stream s) => (State s e -> State s e) -> ParsecT st e s m ()
...
runParsecT p s = unParser p s cok cerr eok eerr
  where
    cok a s' hs st = return (Reply s' Consumed (OK hs a), st)
    cerr err s' st = return (Reply s' Consumed (Error err), st)
    eok a s' hs st = return (Reply s' NotConsumed (OK hs a), st)
    eerr err s' st = return (Reply s' NotConsumed (Error err), st)
...
runParserT ::
  ...
  -- | Input for parser
  s ->
  st -> m (Either (ParseErrorBundle s e) a, st)
runParserT p name s st = snd <$> runParserT' p (initialState name s) st
...
runParserT' ::
  ...
  -- | Initial state
  State s e ->
  st -> m (State s e, (Either (ParseErrorBundle s e) a, st))
runParserT' p s st = do
  (Reply s' _ result, st') <- runParsecT p s st
  let toBundle es =
        ParseErrorBundle
          { bundleErrors =
  ...
  return $ case result of
    OK _ x ->
      case NE.nonEmpty (stateParseErrors s') of
        Nothing -> (s', (Right x, st'))
        Just de -> (s', (Left (toBundle de), st'))
    Error e ->
      (s', (Left (toBundle (e :| stateParseErrors s')), st'))
